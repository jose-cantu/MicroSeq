...
# 3b macOS users only install Expect for live progress bar 
brew install expect  # provides 'unbuffer'
...
| `MICROSEQ_LOG_DIR`  | Where rotated log files are written                               | `logging.dir` from `config.yaml` (default `<repo>/logs`)              | `export MICROSEQ_LOG_DIR=/var/log/microseq` |
...
When neither `MICROSEQ_LOG_FILE` nor `MICROSEQ_LOG_DIR` is set, `setup_logging()` uses `logging.dir` from `config.yaml` when called with `log_dir=None`.
...
GPL-v3
> **Note on VSEARCH:** MicroSeq remains MIT-licensed. Calling the GPL-v3 VSEARCH binary at runtime is fine, but if you redistribute MicroSeq together with the VSEARCH executable you must ship it under GPL-v3 terms (source code + licence text).
